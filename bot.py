...
class Bot(object):
    ...
    def __weather_action(self, nlu_entities=None):
        ...
        weather_speech = self.nlg.weather(
            temperature, current_dtime, "present")
        ...
        if nlu_entities is not None:
            if 'datetime' in nlu_entities:
                if 'grain' in nlu_entities['datetime'][0] and nlu_entities['datetime'][0]['grain'] == 'day':
                    ...
                    if current_dtime.date() == dtime.date():  # hourly weather
                        forecast_obj = {'forecast_type': 'hourly',
                    ...
                    # sometime in the future ... get the weekly forecast/ handle specific days
                        ...
                        forecast_obj = {'forecast_type': 'daily',
            ...
            if 'Weather_Type' in nlu_entities:
                ...
                if weather_type == "current":
                    forecast_obj = {'forecast_type': 'current',
        ...
        weather_data = {"temperature": temperature, "icon": icon,
                        'windSpeed': wind_speed, "hour": datetime.datetime.now().hour}
    ...
    def __youtube_action(self, nlu_entities=None):
        video = None
        DEVELOPER_KEY = "AIzaSyAotWmofSiPMGGTM0oSo8jMMcEUxEmptSQ"  # add developer key
        ...
        if video is not None:
            ...
            body = {'url': videou, 'channel': ch_title,
                    'thumb': thumb, 'titles': title}
            ...
            self.__text_action(
                "I'm sorry, I couldn't understand what video you wanted.")
    ...
    def __zomato_action(self, nlu_entities=None):
        if nlu_entities is not None:
            if 'location' in nlu_entities:
                ...
                print(entities)
                # inset google maps key here
                # inset google maps key here
                response = requests.get(
                    'https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=AIzaSyAx_nPgGLpiiak3Ey0YgDaJjoqlcjZko1A' % entities)
                resp_json_payload = response.json()
                lat = (resp_json_payload['results'][0]
                       ['geometry']['location']['lat'])
                lng = (resp_json_payload['results'][0]
                       ['geometry']['location']['lng'])

                print(lat)
                print(lng)
                locationUrlFromLatLong = "https://developers.zomato.com/api/v2.1/geocode?lat=%s&lon=%s" % (
                    lat, lng)
                header = {"Accept": "application/json",
                          "user_key": "722ef319ddb29144beec1be8cfbd362c"}  # add user key
                ...
                for restaurant in restaurants:
                    ...
                    rating.append(restaurant['restaurant']
                                  ['user_rating']['aggregate_rating'])

                    # print name

                zomato_data = {"cabtype": name,
                               'maxfare': avgcost, "minfare": rating}
                uber_action = "Sure. Here are some results"

                requests.post("http://localhost:8080/zomato",
                              data=json.dumps(zomato_data))
            ...
            self.__text_action(
                "I'm sorry, I couldn't understand what restaurant you wanted.")
    ...
    def __uber_action(self, nlu_entities=None):
        ...
        if entities1 and entities2 is not None:
            response = requests.get(
                'https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=AIzaSyAx_nPgGLpiiak3Ey0YgDaJjoqlcjZko1A' % entities1)  # add key
            resp_json_payload = response.json()

            lat1 = (resp_json_payload['results'][0]
                    ['geometry']['location']['lat'])
            lng1 = (resp_json_payload['results'][0]
                    ['geometry']['location']['lng'])

            response = requests.get(
                'https://maps.googleapis.com/maps/api/geocode/json?address=%s&key=AIzaSyAx_nPgGLpiiak3Ey0YgDaJjoqlcjZko1A' % entities2)  # add key

            resp_json_payload = response.json()

            lat2 = (resp_json_payload['results'][0]
                    ['geometry']['location']['lat'])
            lng2 = (resp_json_payload['results'][0]
                    ['geometry']['location']['lng'])

            oauth2credential = OAuth2Credential(
                client_id='FxkO4ImvByXu-7Dm1vYMHcKIt3C-3-LX ',  # add client id
                access_token='JA.   -HsSUvO2XciOsCC3M25dwxYZTxyr4T9WKRKW8SawMVLucIKNmSZSZtvuE2LUMfb_ykrWxDAAAAIhIo1wfiuri3URzNyQAAABiMGQ4NTgwMy0zOGEwLTQyYjMtODA2ZS03YTRjZjhlMTk2ZWU',  # get access token
                expires_in_seconds='2592000',
                scopes='all_trips delivery history history_lite places profile request request_receipt ride_widgets',
                grant_type='authorization_code',
                redirect_url='http://localhost:8080/redirect_uri',  # add redirect_url
                client_secret='hAjOfJ-OPiQ3dntDT9KtiLxwitcXrxu8-pNPRuzP',  # add client secret
...
# Call REST API
            ...
            cab_data = {"cabtype": type_cab, 'maxfare': bas, "minfare": eta, 'to': entities2, 'from': entities1,
                        'status': status, 'driver': dri_name, 'pic': dri_pic, 'car': car_pix, 'map': ur}
            # print cab_data
    ...
    def __holidays_action(self):
        ...
        self.speech.synthesize_text(
            self.nlg.holiday(next_holiday['localName']))
    ...
    def __module_action(self, nlu_entities=None):
        ...
        if nlu_entities is not None:
            ...
            if 'search_query' in nlu_entities:
                ...
                requests.post("http://localhost:8080/module",
    ...
    def __hound_action(self, speech=None):
        if speech is not None:
            clientId = "Cyvfb5ouBOr6Usalnq9PUQ=="  # get client id from houndify
            # insert client key
            clientKey = "E6bsv7N9EoWCrfZXTZRdsnG5NFnk0XWicqhgMd2SG58gLmCXzVgtuDyVXyvBx18OIeiaBJQ1D248IM8QcOLFEg=="
            userId = "test_user"
            requestInfo = {
                "Latitude": 18.5679,
                "Longitude": 73.9143
            ...
            client = houndify.TextHoundClient(
                clientId, clientKey, userId, requestInfo)
                ...
                if len(response["AllResults"][0]["TemplateData"]["Items"]) > 8:
                    for item in range(0, 10):
                        if 'Title' in response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]:
                            name.append(
                                response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]["Title"])
                        if 'Subtitle' in response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]:
                            rating.append(
                                response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]["Subtitle"])
                        if 'BodyText' in response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]:
                            avgcost.append(
                                response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]["BodyText"])
                        if 'URL' in response["AllResults"][0]["TemplateData"]["Items"][item]["TemplateData"]["Image"]:
                            url.append(response["AllResults"][0]["TemplateData"]
                                       ["Items"][item]["TemplateData"]["Image"]["URL"])

                    hound_data = {"title": name, 'subtitle': rating,
                                  "details": avgcost, "url": url, "command": command}
                    requests.post("http://localhost:8080/other",
                                  data=json.dumps(hound_data))
                else:
                    requests.post("http://localhost:8080/other",
...
#   -H "Authorization: Bearer XIVCS5FFMUMXU2B6R7GXYLGS522BXLDT" \
